...
namespace learn.it.Repos
{
    public class AnswersRepository : IAnswersRepository
    {
        ...
        {
            return await _context.Answers.Where(a => a.User.UserId == userId).Include(a => a.User).ThenInclude(u => u.Permissions).Include(a => a.Flashcard).ThenInclude(f => f.StudySet).ThenInclude(s => s.Creator).ToListAsync();
        }
